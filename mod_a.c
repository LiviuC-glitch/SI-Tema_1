...
void stringSplit_ecb(const char s[], size_t n, int sockfd)
{
	if (n)
	{
		char chunk[n + 1];
...
void stringSplit_cbc(const char s[], size_t n, int sockfd)
{
	int step = 0;
	if (n)
	{
		...
		for (; *s; ++s)
		{
			if (!isspace((unsigned char)*s))
			{
				...
				if (i == n)
				{
					...
					char new_text[MAX], new[MAX];
					xor(chunk, iv, new_text);
					aes_enc(new_text, dec_key, new);
					sprintf(iv, "%s", new);
					printf("%s\n", new_text);
					write(sockfd, new_text, sizeof(new_text));
				}
...
{
	stringSplit_ecb(plain, 15, sockfd);
}

void cbc_enc(char plain[], unsigned char key[16], int sockfd)
{
	stringSplit_cbc(plain, 15, sockfd);
}
